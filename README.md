...
## 2. Tecnologías utilizadas y modularización
...
Primero se actualiza la tabla de tiempos insertando la nueva solve vinculada a su identificador persistido. A continuación, el sistema recalcula todas las estadísticas utilizando exclusivamente los datos almacenados en la base de datos, evitando cualquier cálculo basado en estados temporales del frontend. La tarjeta de detalle no se renderiza automáticamente; únicamente se construye o actualiza cuando el usuario selecciona explícitamente un tiempo dentro de la tabla de tiempos, tomando como referencia el registro persistido correspondiente.
