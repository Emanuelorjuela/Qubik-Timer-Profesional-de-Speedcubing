 ...
 * rotacion(giro, faces)
 * 
 * Rota únicamente la cara correspondiente al movimiento ejecutado.
 * Aplica rotación simple (90°), doble (180°) o antihoraria (-90°)
 * sobre los 8 stickers externos de la cara, preservando el centro.
 */
export const rotacion = (giro, faces) => {

  // Desestructuración de las 6 caras del cubo
  const { white, yellow, green, blue, orange, red } = faces;

  // Índices de una cara 3x3
  const indice = [0, 1, 2, 3, 4, 5, 6, 7, 8];

  // Mapa que asocia cada movimiento con su cara correspondiente
  const colorMap = {
  ...
  // Guarda los valores actuales antes de modificarlos
  const colorf = indice.map((i) => color[i].className);

  // El centro no rota físicamente, se preserva
  const centerClass = colorf[4];

  // Definición de ciclos de rotación según tipo
  const rotaciones = {
  ...
  // Aplicar reemplazo de clases según la secuencia
 ...
 * movesRL(move, faces)
 * 
 * Gestiona rotaciones de las caras L y R,
 * intercambiando columnas laterales entre caras adyacentes.
 */
export const movesRL = (move, faces) => {

  const { white, yellow, green, blue, orange, red } = faces;

  // Se almacenan los valores actuales antes de sobrescribir
  ...
  // Cada bloque define el intercambio correcto según notación estándar
  if (move === "L'") {
    for (let i = 0; i < 3; i++) {
 ...
 * movesUD(move, faces)
 * 
 * Gestiona movimientos U y D intercambiando filas
 * superiores o inferiores entre caras laterales.
 */
 ...
 * movesFB(move, faces)
 * 
 * Gestiona movimientos F y B intercambiando
 * filas y columnas entre caras frontal y trasera.
 */
export const movesFB = (move, faces) => {

  const { white, yellow, green, blue, orange, red } = faces;

  // Cada bloque guarda valores antes del intercambio
  // y aplica la transformación según notación estándar.
